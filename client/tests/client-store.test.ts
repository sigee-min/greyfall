...
test('ClientNetObjectStore: queue future patch and apply in order', () => {
  ...
  const queued = store.applyPatch('obj', 3, [{ op: 'merge', path: 'entries', value: { id: 'b', v: 1 } } as any]);
  assert.equal(queued, true);
  // Now rev2 arrives → apply and then flush rev3
  const ok2 = store.applyPatch('obj', 2, [{ op: 'merge', path: 'entries', value: { id: 'a', v: 2 } } as any]);
  assert.equal(ok2, true);
  const final = store.get('obj')!;
  assert.equal(final.rev, 3);
  const list = (final.value as any).entries as any[];
  const a = list.find((e: any) => e.id === 'a');
  const b = list.find((e: any) => e.id === 'b');
  assert.equal(a.v, 2);
  assert.equal(b.v, 1);
});
...
test('ClientNetObjectStore: insert and remove ops on arrays', () => {
  ...
  // insert single
  store.applyPatch('arr', 2, [{ op: 'insert', path: 'entries', value: { id: 'x', val: 1 } } as any]);
  // insert many
  store.applyPatch('arr', 3, [{ op: 'insert', path: 'entries', value: [{ id: 'y', val: 2 }, { id: 'z', val: 3 }] } as any]);
  // remove by id
  store.applyPatch('arr', 4, [{ op: 'remove', path: 'entries', value: { id: 'y' } } as any]);
  const s = store.get('arr')!;
  const list = (s.value as any).entries as any[];

...
function suggestConcurrency(): number {
  ...
  if (typeof connection.downlink === 'number' && connection.downlink < 1.5) return 1;
  if (connection.effectiveType && /3g|2g|slow-2g/.test(connection.effectiveType)) return 1;
  return 2;
}

export function useAssetPreload(): void {
  ...
  const { manager, attachManager, handleEvent, initialiseSnapshot, clearSnapshot } = useAssetPreloadStore(
    (state) => ({
  ...
  const managerRef = useRef<AssetPreloadManager | null>(null);

...
type OnStalled = (id: string, sinceRev?: number) => void;
...
export class ClientNetObjectStore {
  ...
  private onStalled?: OnStalled;

  constructor(opts?: { onStalled?: OnStalled }) {
  ...
  applyReplace(id: string, rev: number, value: unknown): boolean {
    ...
    if (!q0 || q0.size === 0) this.clearTimer(id);
  ...
  applyPatch(id: string, rev: number, ops: any[]): 'applied' | 'queued' | 'rejected' {
    ...
    if (rev > current.rev + 1) {
      ...
      try { netBus.publish('client:patch:queued', { objectId: id, rev, queuedCount: bucket.size }); } catch {}
      ...
      if (bucket.size > pq.maxQueuedRevs) {
        ...
        try { netBus.publish('client:patch:stalled', { objectId: id, sinceRev: typeof sinceRev === 'number' ? sinceRev : undefined }); } catch {}
        try { this.onStalled?.(id, typeof sinceRev === 'number' ? sinceRev : undefined); } catch {}
        return 'queued';
      }
      this.ensureTimer(id);
      return 'queued';
    ...
    if (rev <= current.rev) {
      ...
      try { netBus.publish('client:patch:applied', { objectId: id, rev: current.rev }); } catch {}
      return 'applied';
    }

    const next = this.applyOps(structuredClone(current.value), ops);
    this.state.set(id, { rev, value: next });
    try { netBus.publish('client:patch:applied', { objectId: id, rev }); } catch {}

...
function cursorUrl(file, fallback) {
...
function elementIsTextual(element) {
    if (element instanceof HTMLTextAreaElement) {
...
function cursorFromClassList(element) {
...
function cursorFromRole(element) {
...
function resolveCursor(target, root) {
...
export function useCustomCursor() {
    useEffect(() => {
        ...
        const applyCursor = (cursor) => {
            ...
            if (next === lastCursor && body.dataset.cursor === next)
                return;
            lastCursor = next;

...
import { selectFullscreenEnabled, selectMusicEnabled, selectMusicVolume, selectPreferencesLoaded, usePreferencesStore } from './store/preferences';
...
const LOBBY_TRACKS = ['/assets/audio/lobby/main-theme.wav', '/assets/audio/lobby/main-theme.mp3'];
function App() {
    const { t } = useI18n();
    useUiSfx();
    useCustomCursor();
    ...
    const [scene, setScene] = useState('mainLobby');
    ...
    const { participants, localParticipantId, sessionMeta, createGame, joinGame, toggleReady, leaveSession, startMissionReady, publishLobbyMessage, registerLobbyHandler } = useSession({ startHostSession, joinHostSession });
    ...
    const { resume: resumeMusic, previewVolume: previewMusicVolume } = useBackgroundMusic(activeTracks, musicPlayEnabled, musicVolume, scene, LOBBY_TRACKS);
    const characterBuilt = useCharacterStore((s) => s.built);
    const [showCharBuilder, setShowCharBuilder] = useState(false);
    useEffect(() => {
        if (scene === 'game' && !characterBuilt)
            setShowCharBuilder(true);
    }, [characterBuilt, scene]);
    const changeScene = useCallback((next) => {
        setScene(next);
        ...
        const unsub = registerLobbyHandler('mission:start', () => {
            try {
                if (fullscreenEnabled && typeof document !== 'undefined') {
                    ...
                    if (!current)
                        void requestFullscreen(document.documentElement, 'mission-start-remote');
                }
            }
            catch { }
            if (scene !== 'game')
    ...
    useEffect(() => {
        ...
        const handleError = (event) => console.warn('[fullscreen] error event', event);
    ...
    const handleCreateGame = useCallback(async (name) => {
    ...
    const handleJoinGame = useCallback(async (name, code) => {
        if (fullscreenEnabled && typeof document !== 'undefined' && !getFullscreenElement()) {
    ...
    const handleManualAnswer = useCallback(async (code) => {
        if (!sessionMeta || sessionMeta.mode !== 'host')
            return;
        try {
    ...
    const content = useMemo(() => {
        if (scene === 'mainLobby') {
            return (_jsx(GameLobby, { playerName: playerName, onNameChange: setPlayerName, onCreate: handleCreateGame, onJoin: handleJoinGame, onOptions: () => setOptionsOpen(true), onAbout: () => setDeveloperOpen(true), background: "/assets/bg/theme.png" }));
        }
        if (scene === 'startLobby' && sessionMeta) {
            const signalSessionId = sessionMeta.signalSessionId ?? null;
            ...
            const acceptAnswer = sessionMeta.mode === 'host' && !autoConnect ? handleManualAnswer : undefined;
            return (_jsx(GameStartLobby, { background: "/assets/bg/lobby.gif", mode: sessionMeta.mode, lobbyCode: sessionMeta.code, answerCode: answerCode, participants: participants, localParticipantId: localParticipantId, canSendChat: canSendChat, channelReady: channelOpen, chatMessages: chatMessages, onToggleReady: toggleReady, onStartGame: startMission, onLeave: handleLeave, onAcceptAnswer: acceptAnswer, autoConnect: autoConnect, onOptions: () => setOptionsOpen(true), onSendChat: sendChatMessage, 
                // 매니저는 컴포넌트 내부 기본값 사용 (기본: 'smart')
                publishLobbyMessage: publishLobbyMessage, probeChannel: probeChannel }));
        }
        const stageBg = scene === 'game' ? worldMedia.bgSrc : '/assets/bg/stage-ops.png';
        return (_jsxs(StageViewport, { background: stageBg, className: "cursor-crosshair", children: [_jsxs("div", { className: "pointer-events-none absolute inset-0", children: [_jsx(Toaster, {}), _jsx("div", { className: "pointer-events-none" }), _jsxs("div", { className: "pointer-events-auto mx-6 mt-6 flex items-center justify-between rounded-2xl border border-border/60 bg-card/70 px-6 py-4 backdrop-blur supports-[backdrop-filter]:bg-card/50", children: [_jsxs("div", { children: [_jsx("p", { className: "text-xs uppercase tracking-[0.4em] text-muted-foreground", children: t('stage.brand') }), _jsxs("h1", { className: "text-2xl font-semibold", children: [t('lobby.title.line1'), " ", t('lobby.title.line2')] })] }), _jsxs("div", { className: "flex items-center gap-6", children: [_jsxs("div", { className: "flex gap-4 text-sm font-semibold", children: [_jsxs("span", { className: "text-primary", children: [t('resources.glow'), " ", resources.glow] }), _jsxs("span", { className: "text-destructive", children: [t('resources.corruption'), " ", resources.corruption] })] }), _jsx("button", { type: "button", className: "rounded-md border border-border bg-background/70 px-3 py-2 text-xs font-semibold uppercase tracking-[0.28em] text-muted-foreground transition hover:border-primary hover:text-primary", onClick: () => setOptionsOpen(true), children: t('common.options') }), _jsx(ControlDock, {})] })] }), _jsxs("div", { className: "pointer-events-auto absolute bottom-6 left-6 flex w-[360px] flex-col gap-4", children: [_jsx(ChatDock, {}), _jsx(CommandConsole, { publish: publishLobbyMessage, localParticipantId: localParticipantId })] }), scene === 'game' && (_jsxs(_Fragment, { children: [_jsxs("div", { className: "pointer-events-auto absolute bottom-6 right-6 w-[360px]", children: [_jsx(FieldGraph, { localParticipantId: localParticipantId, publish: publishLobbyMessage }), _jsx("div", { className: "mt-4" }), _jsx(MapMini, { localParticipantId: localParticipantId, participants: participants, publish: publishLobbyMessage, register: registerLobbyHandler })] }), _jsx("div", { className: "pointer-events-auto absolute bottom-6 left-6 w-[360px]", children: _jsx(InteractionPanel, { localParticipantId: localParticipantId, participants: participants, publish: publishLobbyMessage, register: registerLobbyHandler }) })] }))] }), scene === 'game' && (_jsx(SettingsOverlay, { open: settingsOpen, onClose: () => setSettingsOpen(false) })), scene === 'game' && showCharBuilder && (_jsx(CharacterBuilder, { onClose: () => setShowCharBuilder(false), playerName: playerName, localParticipantId: localParticipantId, publish: publishLobbyMessage }))] }));
    }, [
        handleCreateGame,
        handleJoinGame,
        handleLeave,
    ...
    useEffect(() => {
        setToolsProviders({
            getChatHistory: async (limit, _includeSystem) => {
    ...
    return (_jsxs(_Fragment, { children: [content, _jsx(OptionsDialog, { open: optionsOpen, onClose: handleOptionsClose, scene: scene, onEnableMusic: resumeMusic, onPreviewMusicVolume: previewMusicVolume }), _jsx(DeveloperDialog, { open: developerOpen, onClose: () => setDeveloperOpen(false) }), _jsx(NetworkMonitorDialog, { open: netmonOpen, onClose: () => setNetmonOpen(false) }), debugPageEnabled && llmMonitorOpen && _jsx(LlmMonitor, { onClose: () => setLlmMonitorOpen(false) }), _jsx(ErrorDialog, { open: Boolean(errorMessage), message: errorMessage ?? '', onClose: dismissError })] }));
}
export default App;

...
function createEntry(url, type, note) {
    return { url, type, note };
}
function collectWorldAssets() {
    const backgrounds = new Set();
    const audio = new Set();
    for (const map of WORLD_STATIC.maps) {
        if (map.bg?.path)
            backgrounds.add(map.bg.path);
        if (map.music?.tracks) {
...
const worldBackgrounds = worldBackgroundsRaw.filter((url) => SUPPORTED_IMAGE_EXTENSIONS.has(getExtension(url)));
const worldAudio = worldAudioRaw.filter((url) => SUPPORTED_AUDIO_EXTENSIONS.has(getExtension(url)));
const WORLD_BACKGROUND_ENTRIES = worldBackgrounds.map((url) => createEntry(url, 'image', 'World background'));
...
export function listAllPreloadEntries(priorities = ['critical', 'recommended', 'deferred']) {
    const seen = new Set();
    const out = [];

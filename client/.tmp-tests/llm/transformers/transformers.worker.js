...
function emitProgress(text, progress) {
    self.postMessage({ type: 'progress', text, progress });
}
    ...
    switch (msg.type) {
        case 'init': {
            ...
            try {
                ...
                try {
                    const origin = self.location?.origin ?? '(unknown)';
                    const isolated = self.crossOriginIsolated === true;
                    const sabAvail = typeof self.SharedArrayBuffer !== 'undefined';
                    let sabAlloc = false;
                    try {
                        if (sabAvail) {
                            const b = new self.SharedArrayBuffer(16);
                            sabAlloc = b.byteLength === 16;
                        }
                    }
                    catch { }
                ...
                const withProgress = { device: 'wasm', dtype, progress_callback: (data) => {
                        ...
                        switch (status) {
                            ...
                            case 'progress': {
                                ...
                                if (file) {
                                    if (lp && tp && typeof pct === 'number')
                                        text = `다운로드 진행: ${file} ${lp}/${tp} (${pct}%)`;
                                    else if (lp && tp)
                                        text = `다운로드 진행: ${file} ${lp}/${tp}`;
                                    else if (lp && typeof pct === 'number')
                                        text = `다운로드 진행: ${file} ${lp} (${pct}%)`;
                                    else if (typeof pct === 'number')
                                        text = `다운로드 진행: ${file} ${pct}%`;
                                    else
                                        text = `다운로드 진행: ${file}`;
                                }
                                else {
                                    if (lp && tp && typeof pct === 'number')
                                        text = `다운로드 진행: ${lp}/${tp} (${pct}%)`;
                                    else if (lp && tp)
                                        text = `다운로드 진행: ${lp}/${tp}`;
                                    else if (lp && typeof pct === 'number')
                                        text = `다운로드 진행: ${lp} (${pct}%)`;
                        ...
                        self.postMessage({ type: 'progress', text, progress: typeof pct === 'number' ? (pct / 100) : undefined });
                    } };
                try {
                    ...
                    if (dtype === 'q8') {
                        try {
                            hfGenerator = await pipeline('text-generation', hfModelId, { ...withProgress, dtype: 'q4' });
                        }
                        catch { }
                    }
                    if (!hfGenerator)
                ...
                initialised = true;
                try {
                    self.postMessage({ type: 'progress', text: 'ready', progress: 1 });
                }
                catch { }
            }
            catch (e) {
                ...
                self.postMessage({ type: 'progress', text: msg, progress: 0.21 });
        ...
        case 'run': {
            if (!initialised || !hfGenerator) {
                self.postMessage({ type: 'error', id: msg.id, error: 'Transformers engine not initialised' });
                return;
            }
            let streamed = '';
            const streamer = new TextStreamer(hfGenerator.tokenizer, {
                skip_prompt: true,
                ...
                callback_function: (text) => {
                    if (aborted.has(msg.id))
                        return;
                    streamed += text;
                    self.postMessage({ type: 'token', id: msg.id, token: text });
                }
            });
            try {
                const messages = [
                ...
                const out = await hfGenerator(messages, opts);
                if (!aborted.has(msg.id)) {
                    const text = String((out?.[0]?.generated_text?.at?.(-1)?.content) ?? streamed ?? '');
                    self.postMessage({ type: 'done', id: msg.id, text });
                }
                else {
                ...
                self.postMessage({ type: 'error', id: msg.id, error: err });
            }
            finally {
                    ...
                    if (anySelf.indexedDB && anySelf.indexedDB.databases) {
                        ...
                        if (Array.isArray(dbs)) {
                            for (const db of dbs) {
                                ...
                                if (name) {
                                    try {
                                        await new Promise((res) => { const req = anySelf.indexedDB.deleteDatabase(name); req.onsuccess = () => res(null); req.onerror = () => res(null); req.onblocked = () => res(null); });
                                    }
                                    catch { }
                                }
                            }

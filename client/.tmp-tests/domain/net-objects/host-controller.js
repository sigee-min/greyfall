...
import { HostAckFallback, attachHostObject, getNetObjectDescriptor, getNetObjectDescriptors, subscribeNetObjectDescriptors, registerNetObject } from './registry.js';
...
export class HostNetController {
    constructor({ publish, lobbyStore, busPublish, getPeerIds, sendToPeer, provideDescriptors }) {
        this.ackTimers = new Map();
        this.lastSentPerPeer = new Map();
        this.ackedRevPerPeer = new Map();
    ...
    scheduleAck(peerId, id, rev, attempt = 1) {
        ...
        try {
            netBus.publish('net:ack:scheduled', { peerId, objectId: id, rev, attempt });
        }
        catch { }
        const timer = setTimeout(() => {
            ...
            if (!hostObject) {
                if (attempt < 3)
                    this.scheduleAck(peerId, id, rev, attempt + 1);
                else
            ...
            if (peerId && this.sendToPeer) {
                ...
                if (incrementalLimit != null && hostObject.getLogsSince) {
                    ...
                    if (pending.length > 0 && pending.length <= incrementalLimit) {
                        for (const entry of pending) {
                            const msg = createLobbyMessage('object:patch', { id, rev: entry.rev, ops: entry.ops });
                            this.sendToPeer(peerId, msg);
                        }
                        try {
                            netBus.publish('net:ack:fallback', { peerId, objectId: id, rev, strategy: 'incremental' });
                        }
                        catch { }
                ...
                if (!sentIncremental) {
                    ...
                    if (fallback === HostAckFallback.Snapshot) {
                        ...
                        if (snapshot) {
                            const msg = createLobbyMessage('object:replace', { id, rev, value: snapshot.value });
                            this.sendToPeer(peerId, msg);
                        }
                        else {
                        ...
                        try {
                            netBus.publish('net:ack:fallback', { peerId, objectId: id, rev, strategy: 'snapshot' });
                        }
                        catch { }
                    }
                        ...
                        hostObject.onRequest(undefined);
                        try {
                            netBus.publish('net:ack:fallback', { peerId, objectId: id, rev, strategy: 'onRequest' });
                        }
                        catch { }
                    }
                        ...
                        // no-op
                        try {
                            netBus.publish('net:ack:fallback', { peerId, objectId: id, rev, strategy: 'none' });
                        }
                        catch { }
                    }
                }
    ...
    resolveAck(peerId, id, rev) {
        ...
        if (t) {
            ...
            try {
                netBus.publish('net:ack:resolved', { peerId, objectId: id, rev });
            }
            catch { }
        }
        else {
        ...
        if (peerId)
            this.ackedRevPerPeer.set(`${peerId}:${id}`, rev);
    }

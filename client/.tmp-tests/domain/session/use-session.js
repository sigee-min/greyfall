...
import { createLobbyMessage } from '../../protocol';
import { useLobbyStore } from './session-store';
...
import { isLikelySignalCode, requestSignalSessionId, SIGNAL_SERVER_AVAILABLE, useSignalBridge } from './signal-bridge';
export function useSession({ startHostSession: startHost, joinHostSession: joinHost }) {
    ...
    const leaveSession = useCallback(() => {
        ...
        void (async () => {
            ...
            try {
                ...
                if (typeof bus.clearProgress === 'function')
                    bus.clearProgress();
            }
            catch { /* ignore */ }
        })();
        if (modeRef.current === 'guest') {
            ...
            if (localId) {
                try {
                    ...
                    console.warn(`[lobby] failed to send leave inline ${String(error?.message || error)}`);
                }
            }
        }
    ...
    const scheduleDisconnectTimeout = useCallback((reason, ms = 40000) => {
    ...
    const requestIceRestartHost = useCallback(async (reason) => {
            ...
            try {
                manager.forEach(async ({ peerId }) => {
                    try {
                        ...
                        if (!ok)
                            console.warn('[signal] failed to send per-guest restart offer', { peerId });
                    }
                    catch (err) {
        ...
        if (!current)
            return;
        if (renegotiateInFlightRef.current)
            return;
        if (renegotiateAttemptsRef.current >= 5)
            return;
    ...
    const attachPeerWatchers = useCallback((peer, role) => {
        const handleConn = () => {
    ...
    const publishLobbyMessage = useCallback((kind, body, context = 'external') => {
        ...
        if (modeRef.current === 'host' && hostPeersRef.current) {
            ...
            const hostHandledKinds = new Set([
                'chat:append:request',
                'field:move:request',
                'map:travel:propose',
                'map:travel:vote',
            ...
            if (kind === 'llm:progress') {
                hostControllerRef.current?.ingest(envelope);
            }
            else if (hostHandledKinds.has(kind)) {
                hostControllerRef.current?.ingest(envelope);
                return true;
            }
            hostPeersRef.current.sendAll(envelope);
            const logMsg = `[lobby] broadcast context=${context} kind=${kind}`;
        ...
        const session = sessionRef.current;
        if (!session) {
            console.warn(`[lobby] send skipped – no session context=${context} kind=${kind}`);
            return false;
        }
        ...
        const logMsg = `[lobby] send context=${context} kind=${kind}`;
        if (kind === 'llm:progress' || context === 'llm-progress')
            console.debug(logMsg);
        else
            console.info(logMsg);
    ...
    const registerLobbyHandler = useCallback((kind, handler) => lobbyBus.subscribe(kind, handler), [lobbyBus]);
    ...
    const handleChannelClose = useCallback((event) => {
        console.warn(`[lobby] channel closed mode=${modeRef.current} state=${event?.type ?? 'unknown'}`);
    ...
    const handleChannelError = useCallback((event) => {
    ...
    const createGame = useCallback(async (name) => {
        ...
        const relayIceCandidate = (_candidate) => {
            return;
        };
        ...
        try {
            ...
            hostControllerRef.current = new HostNetController({
                publish: (kind, body, context) => publishLobbyMessage(kind, body, context),
                lobbyStore,
        ...
        const hostParticipant = { id, name, tag, ready: false, role: 'host' };
    ...
    const joinGame = useCallback(async (name, code) => {
        ...
        const offerPromise = createDeferred();
            ...
            await signalBridge.connect(upperInput, 'guest', {
                ...
                onOffer: async ({ code: offerCode }) => {
                    ...
                    try {
                        ...
                        if (active.peer.signalingState !== 'stable') {
                            try {
                                await active.peer.setLocalDescription({ type: 'rollback' });
                            }
                            catch (err) {
                ...
                onCandidate: async ({ candidate: candidateJson }) => {
                    ...
                    try {
                        ...
                        await sessionRef.current?.peer.addIceCandidate(candidate);
                    }
                    catch (error) {
        ...
        let session;
        try {
            session = await joinHost(joinCode, events, (answerCode) => {
        ...
        const self = { id, name, tag, ready: false, role: 'guest' };
    ...
    const toggleReady = useCallback((participantId) => {
        ...
        const updated = raw.map((participant) => participant.id === participantId ? { ...participant, ready: nextReady } : participant);
    ...
    const startMissionReady = useMemo(() => lobbyStore.participants.length > 0 && lobbyStore.participants.every((participant) => participant.ready), [lobbyStore.participants]);
    useEffect(() => {

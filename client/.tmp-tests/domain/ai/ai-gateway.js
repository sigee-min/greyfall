...
import { buildSystemFromSections, planDirectives, narrateDirectives } from '../../llm/spec/prompts';
import { buildEligibilitySections } from './gateway/eligibility';
const DEBUG = Boolean(import.meta.env?.VITE_LLM_DEBUG);
function getLastOutputText(scratch) {
...
export async function requestAICommand(params) {
    const { manager, requestType, userInstruction, persona, locale, contextText, temperature = 0.5, maxTokens, fallbackChatText = '문제가 발생했습니다.', timeoutMs } = params;
    return runWithManagerLock(manager, async () => {
        if (DEBUG)
            console.debug(`[gw] start manager=${manager} hasContext=${Boolean(contextText)} temp=${temperature} maxTokens=${maxTokens ?? 'auto'} timeout=${timeoutMs ?? 'auto'}`);
        const { maxTokens: maxTok, effectiveTimeout } = resolveGatewayConfig(manager, {
            maxTokens,
            timeoutMs,
            twoPhase: false
        });
        const runtimeTimeout = Math.max(3000, Math.min(180000, effectiveTimeout));
        if (DEBUG)
            console.debug('[gw] ensureRuntimeReady.begin');
        await ensureRuntimeReady(manager);
        if (DEBUG)
            console.debug('[gw] ensureRuntimeReady.ok');
        const ctx = {
        ...
        try {
            ...
            const buildSections = async () => {
                let historyText = '';
                try {
                    const res = await ctx.tools?.invoke('chat.history', { limit: 10 });
                    if (res?.ok)
                        historyText = formatHistory(res.data.items);
                }
                catch (err) {
                    if (DEBUG)
                        console.debug('[gw] chat.history failed', formatError(err));
                }
                ...
                if (params.eligibility) {
                    try {
                        ...
                        if (DEBUG)
                            console.debug('[gw] eligibility build failed', formatError(e));
                    }
                }
                if (contextText && contextText.trim())
                    s.context = contextText;
                if (historyText)
                    s.recentChat = historyText;
                if (!s.requester && params.actorId)
                    s.requester = `actor=${params.actorId} (self)`;
                return s;
            };
            let start;
            if (requestType === 'chat') {
                ...
                start = { id: 'scene.detail', nodeId: 'scene.detail', params: async () => {
                        const sections = await buildSections();
                        const systemSuffix = buildSystemFromSections('', sections);
                        const directive = '지정된 요소를 1–2문장 확장';
                        return { persona: defaultPersona, systemSuffix, userSuffix: '', directive };
                    }, next: null };
            }
                ...
                start = { id: 'turn.summarize', nodeId: 'turn.summarize', params: async () => {
                        const sections = await buildSections();
                        const systemSuffix = buildSystemFromSections('', sections);
                        const directive = '핵심을 bullet 2–3개로 요약하고 JSON 한 줄로 내보냅니다.';
                        return { persona: defaultPersona, systemSuffix, userSuffix: '', directive };
                    }, next: null };
            }
                ...
                start = { id: 'session.summarize', nodeId: 'session.summarize', params: async () => {
                        const sections = await buildSections();
                        const systemSuffix = buildSystemFromSections('', sections);
                        const directive = '세션 저장용 2–3문장 요약';
                        return { persona: defaultPersona, systemSuffix, userSuffix: '', directive };
                    }, next: null };
            }
                ...
                start = { id: 'npc.reply', nodeId: 'npc.reply', params: async () => {
                        const sections = await buildSections();
                        const systemSuffix = buildSystemFromSections('', sections);
                        const directive = 'NPC 말투로 1–2문장으로 답변합니다.';
                        return { persona: defaultPersona, systemSuffix, userSuffix: '', directive };
                    }, next: null };
            }
                ...
                start = { id: 'npc.name', nodeId: 'npc.name', params: async () => {
                        const sections = await buildSections();
                        const systemSuffix = buildSystemFromSections('', sections);
                        const directive = 'JSON 한 줄 {"names":["..",".."]}';
                        return { persona: defaultPersona, systemSuffix, userSuffix: '', directive };
                    }, next: null };
            }
                ...
                start = { id: 'entity.link', nodeId: 'entity.link', params: async () => {
                        const sections = await buildSections();
                        const systemSuffix = buildSystemFromSections('', sections);
                        const directive = '지칭을 액터 ID로 매핑하고 JSON 한 줄로 출력';
                        return { persona: defaultPersona, systemSuffix, userSuffix: '', directive };
                    }, next: null };
            }
                ...
                start = { id: 'intent.disambiguate', nodeId: 'intent.disambiguate', params: async () => {
                        const sections = await buildSections();
                        const systemSuffix = buildSystemFromSections('', sections);
                        const directive = '확인 질문과 2–4개 선택지를 JSON 한 줄로 출력';
                        return { persona: defaultPersona, systemSuffix, userSuffix: '', directive };
                    }, next: null };
            }
                ...
                start = { id: 'turn.suggest', nodeId: 'turn.suggest', params: async () => {
                        const sections = await buildSections();
                        const systemSuffix = buildSystemFromSections('', sections);
                        const directive = '다음 전개 제안 2–3개를 JSON 한 줄로 출력';
                        return { persona: defaultPersona, systemSuffix, userSuffix: '', directive };
                    }, next: null };
            }
                ...
                start = { id: 'scene.hazard.tag', nodeId: 'scene.hazard.tag', params: async () => {
                        const sections = await buildSections();
                        const systemSuffix = buildSystemFromSections('', sections);
                        const directive = '위험을 최대 2개 선택하여 JSON 한 줄로 출력';
                        return { persona: defaultPersona, systemSuffix, userSuffix: '', directive };
                    }, next: null };
            }
                ...
                start = { id: 'safety.screen', nodeId: 'safety.screen', params: async () => {
                        const sections = await buildSections();
                        const systemSuffix = buildSystemFromSections('', sections);
                        const directive = 'JSON 한 줄 {"flag":true|false,"reasons":[".."],"suggest":".."}';
                        return { persona: defaultPersona, systemSuffix, userSuffix: '', directive };
                    }, next: null };
            }
            else {
            ...
            if (DEBUG)
                console.debug(`[gw] pipeline.begin node=${start.nodeId}`);
            const task = requestType; // tag stream meta with requestType for export mapping
            const exec = async (msgs, opts) => {
                if (DEBUG)
                    console.debug(`[gw.exec] run messages=${msgs.length} temp=${opts.temperature} maxTokens=${opts.maxTokens} timeout=${opts.timeoutMs} task=${task}`);
                const out = await generateMessagesWithTimeout(manager, msgs, { ...opts, locale, task });
                if (DEBUG)
                    console.debug(`[gw.exec] done chars=${out.length}`);
                return out;
            };
            const done = await runPipeline({ start, ctx, registry: InMemoryNodeRegistry, exec });
            if (DEBUG)
                console.debug('[gw] pipeline.done');
            const text = getLastOutputText(done.scratch) ?? fallbackChatText;
            if (DEBUG)
                console.debug(`[gw] out chars=${text.length}`);
            return { cmd: requestType, body: text };
        }
        catch (e) {
            if (DEBUG)
                console.debug('[gw] error', formatError(e));

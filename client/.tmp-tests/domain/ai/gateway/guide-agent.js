...
export function useGuideAgent({ enabled, manager, registerLobbyHandler, publishLobbyMessage, localParticipantId, participants: _participants, locale = 'ko', policy }) {
    ...
    useEffect(() => {
        ...
        const unsubscribe = registerLobbyHandler('chat', async (message) => {
            ...
            if (!shouldReply(entry.body, guideName, fullPolicy))
                return;
            const now = Date.now();
            if (now - lastAtRef.current < fullPolicy.cooldownMs)
                return;
...
function pushHistory(ref, item, max) {
    ref.current = [...ref.current, item].slice(-max);
}
function shouldReply(text, name, policy) {
    if (!policy.respondOnMention)
        return false;
    const t = text.toLowerCase();
    const nameLc = name.toLowerCase();
    if (t.includes(nameLc))
        return true;
    if (t.includes('@' + nameLc))
        return true;
...
function buildEligibilityFromLiveState(requesterParticipantId, participants, loadoutsById) {
    const positions = worldPositionsClient.getAll();
    const posById = new Map(positions.map((p) => [p.id, p]));
    const actorsState = worldActorsClient.getAll();
    const invMap = {};
    if (Array.isArray(actorsState)) {
        for (const a of actorsState) {
            if (Array.isArray(a.inventory) && a.inventory.length)
                invMap[`p:${a.id}`] = a.inventory.map((i) => ({ key: String(i.key), count: Math.max(0, Math.floor(i.count || 0)) }));
        }
    }
    ...
    if (reqLoadout) {
        ...
        if (selfIdx >= 0) {
            ...
            const a = elig.actors[selfIdx];
            const meta = [];
            if (traitsLabel)
                meta.push(`traits=[${traitsLabel}]`);
            if (passivesLabel)
                meta.push(`passives=[${passivesLabel}]`);
            if (meta.length) {

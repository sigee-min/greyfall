export const ChatHistoryTool = {
    ...
    inputGuard: (input) => {
        const i = (input || {});
        if (i.limit != null && (typeof i.limit !== 'number' || i.limit < 1 || i.limit > 10))
            throw new Error('limit:1..10');
        if (i.includeSystem != null && typeof i.includeSystem !== 'boolean')
            throw new Error('includeSystem:boolean');
    },
    outputGuard: (data) => {
    ...
    async invoke(ctx, input) {
        ...
        if (!prov)
            return { ok: false, error: 'no-provider:getChatHistory' };

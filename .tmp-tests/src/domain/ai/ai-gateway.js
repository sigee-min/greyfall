...
export async function requestAICommand(params) {
    const { manager, requestType, userInstruction, persona, contextText, temperature = 0.5, maxTokens, fallbackChatText = '문제가 발생했습니다.', timeoutMs } = params;
    return runWithManagerLock(manager, async () => {
        ...
        const ctx = {
            ...
            manager,
            scratch: { overrides: { temperature, maxTokens: maxTok, timeoutMs: Math.min(60000, Math.max(3000, effectiveTimeout)) } },
            tools: makeDefaultToolsHost({ manager, providers: getToolsProviders() })
        };
        const exec = async (msgs, opts) => {
            if (DEBUG)
                console.debug(`[gw.exec] run messages=${msgs.length} temp=${opts.temperature} maxTokens=${opts.maxTokens} timeout=${opts.timeoutMs}`);
                ...
                start = {
                    ...
                    params: async (c) => {
                        ...
                        if (contextText && contextText.trim())
                            parts.push(`맥락:\n${contextText}`);
                        if (historyText)
                            parts.push(`최근 채팅(최대 10개):\n${historyText}`);
                        if (DEBUG)
                            console.debug(`[gw] chat.params hasHistory=${Boolean(historyText)} hasContext=${Boolean(contextText)}`);
                        return {

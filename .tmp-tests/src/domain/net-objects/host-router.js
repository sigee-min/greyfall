...
export class HostRouter {
    constructor(args) {
        ...
        this.limiter = args.limiter ?? new SlidingWindowLimiter(5, 10000);
        ...
        try {
            switch (message.kind) {
                ...
                case 'chat:append:request': {
                    ...
                    const rawBody = String(message.body.body ?? '');
                    const trimmed = rawBody.trim();
                    try {
                        console.debug('[/llm] chat:append:request recv', { authorId, body: trimmed.slice(0, 120) });
                    }
                    catch { }
                    if (!trimmed)
                        break;
                    ...
                    if (llmMatch) {
                        ...
                        if (!prompt)
                            break;
                        try {
                            console.debug('[/llm] prompt parsed', { len: prompt.length, preview: prompt.slice(0, 120) });
                        }
                        catch { }
                        ...
                        try {
                            console.debug('[/llm] prompt echoed');
                        }
                        catch { }
                        ...
                        void (async () => {
                            try {
                                try {
                                    console.debug('[/llm] gateway.request start');
                                }
                                catch { }
                                ...
                                try {
                                    console.debug('[/llm] gateway.request done', { cmd: ai?.cmd });
                                }
                                catch { }
                                try {
                                    console.debug('[/llm] execAI start');
                                }
                                catch { }
                                await executeAICommand(ai, {
                                    manager: 'smart',
                                    publishLobbyMessage: (k, b, c) => this.send(k, b, c),
                                    participants: this.lobbyStore.participantsRef.current,
                                    localParticipantId: this.lobbyStore.localParticipantIdRef.current
                                });
                                try {
                                    console.debug('[/llm] execAI done');
                                }
                                catch { }
                            }
                            catch (err) {
                ...
                case 'map:travel:request': {
                    const { requesterId, direction, toMapId } = message.body;
                    ...
                    if (members.length === 0)
                        break;
                    const posState = this.world.replicator?.get?.('world:positions')?.value;
                    const list = Array.isArray(posState?.list) ? posState.list : [];
                    ...
                    const ok = this.party.travel(direction, toMapId);
                    if (!ok)
                        console.warn('[travel] failed', { direction, toMapId });
                    break;
                }
                case 'map:travel:propose': {
                    const { requesterId, direction, toMapId, quorum } = message.body;
                    if (!this.limiter.allow(`travel:${requesterId}`)) {
                        console.warn('[travel] rate limited', { requesterId });
                        break;
                    }
                    ...
                    this.send('map:travel:update', { inviteId, status: 'proposed', targetMapId: targetId, yes, no, total, quorum: q }, 'travel:update');
                    ...
                    this.travelPollTimer = setTimeout(() => {
                        ...
                        const { yes, no, total } = this.computeVoteCounts();
                        if (status === 'approved') {
                            const succeeded = this.party.travel(undefined, this.travelPoll.targetMapId);
                            if (!succeeded) {
                                this.send('map:travel:update', { inviteId, status: 'rejected', targetMapId: this.travelPoll.targetMapId, yes, no, total, quorum: this.travelPoll.quorum }, 'travel:update');
                                this.travelPoll = null;
                                if (this.travelPollTimer) {
                                    clearTimeout(this.travelPollTimer);
                                    this.travelPollTimer = null;
                            ...
                            this.send('map:travel:update', { inviteId, status: 'approved', targetMapId: this.travelPoll.targetMapId, yes, no, total, quorum: this.travelPoll.quorum }, 'travel:update');
                        }
                        else {
                            this.send('map:travel:update', { inviteId, status: 'rejected', targetMapId: this.travelPoll.targetMapId, yes, no, total, quorum: this.travelPoll.quorum }, 'travel:update');
                        }
                ...
                case 'map:travel:vote': {
                    if (!this.travelPoll)
                        break;
                    const { inviteId, voterId, approve } = message.body;
                    if (inviteId !== this.travelPoll.inviteId)
                        break;
                    if (!this.travelPoll.votes.has(String(voterId)))
                        break;
                    this.travelPoll.votes.set(String(voterId), Boolean(approve));
                    const status = this.evaluateTravelVote();
                    if (status === 'pending') {
                        const { yes, no, total } = this.computeVoteCounts();
                        this.send('map:travel:update', { inviteId, status: 'proposed', targetMapId: this.travelPoll.targetMapId, yes, no, total, quorum: this.travelPoll.quorum }, 'travel:update');
                    }
                ...
                case 'map:travel:cancel': {
                    if (!this.travelPoll)
                        break;
                    const { inviteId, byId } = message.body;
                    if (inviteId !== this.travelPoll.inviteId)
                        break;
                    ...
                    if (!isHost)
                        break;
                    const { yes, no, total } = this.computeVoteCounts();
                    this.send('map:travel:update', { inviteId, status: 'cancelled', targetMapId: this.travelPoll.targetMapId, yes, no, total, quorum: this.travelPoll.quorum }, 'travel:update');
                    this.travelPoll = null;
                    if (this.travelPollTimer) {
                        clearTimeout(this.travelPollTimer);
                        this.travelPollTimer = null;
                ...
                case 'interact:invite': {
                    const { inviteId, fromId, toId, mapId, fieldId, verb } = message.body;
                    // Validate same field
                    const posState = this.world.replicator?.get?.('world:positions')?.value;
                    const list = Array.isArray(posState?.list) ? posState.list : [];
                    ...
                    this.send('interact:invite', { inviteId, fromId, toId, mapId, fieldId, verb }, 'interact:invite');
                    break;
                }
                case 'interact:accept': {
                    const { inviteId, toId } = message.body;
                    // No further validation here; in a real system we would check invite store
                    this.send('interact:confirmed', { inviteId, fromId: 'unknown', toId, verb: 'unknown' }, 'interact:confirmed');
                    break;
                }
                case 'interact:cancel': {
                    const { inviteId, byId } = message.body;
                    this.send('interact:cancel', { inviteId, byId }, 'interact:cancel');
                    break;
                }
                case 'object:ack': {
